...
function UsageSectionContent() {
  ...
  const [order, setOrder] = React.useState<string>("default");
  ...
  const [sortByName, setSortByName] = React.useState(order === "name");
    ...
    <Section className="usage-section">
      <Container>
        ...
        <div className="kto-grid kto-grid-gap-16 kto-offset-top-48">
          {sortedTestimonials.map((item: CompanyData, index: number) => (
            <a
              key={index}
              href={item.url}
              target="_blank"
